...
int NoMemoryDiffs = 0;  // When 1, don't save memory diff files (visual-only mode)
...
void Automation_OnFrame(int frameCount, void* screen, int mode, int Hmode, int Vmode)
{
    ...
    else
    {
        ...
        // Don't save memory diffs if NoMemoryDiffs is enabled (visual-only mode)
        // Also respect SaveMemoryOnlyAfterVisual flag
        bool shouldCheckMemory = (NoMemoryDiffs == 0) &&
                                ((SaveMemoryOnlyAfterVisual == 0) || (DiffCount > 0));

        // Debug: log first frame to confirm NoMemoryDiffs is working
        static bool loggedOnce = false;
        if (!loggedOnce && frameCount > 0) {
            loggedOnce = true;
            char debugPath[1024];
            sprintf(debugPath, "%s\\__debug_mode.txt", ScreenshotDir);
            FILE* dbg = fopen(debugPath, "w");
            if (dbg) {
                fprintf(dbg, "NoMemoryDiffs=%d, shouldCheckMemory=%d\n", NoMemoryDiffs, shouldCheckMemory ? 1 : 0);
                fclose(dbg);
            }
        }
        ...
        // (unless NoMemoryDiffs mode - visual-only, no state dumps)
        if (screenshotDiff && !memoryDiff && NoMemoryDiffs == 0)
        {
            StateDump_DumpStateToFile(ScreenshotDir, basename);
        }
